...
from ivfs_config import (FIT_WINDOW_HOURS, HIGGS_GZ, HIGGS_TXT, HIGGS_URL, IVF_PARAM_BOUNDS, PHI, PSI,
                         SMOOTH_WINDOW, TAU_PARAM_BOUNDS, ensure_layout)
...
def build_hourly_curve(rt_timestamps: np.ndarray,
    ...
    if re_timestamps is not None and len(re_timestamps) > 0:
        ...
        smoothed = np.convolve(re_norm, kernel, mode='same')
        raw_ratio = re_window / (window_counts + 1.0)
        ratio_smoothed = np.convolve(raw_ratio, kernel, mode='same')
        result['re_window'] = re_window
        result['reply_proxy'] = np.clip(smoothed, 0.0, 1.0)
        result['reply_ratio'] = np.clip(ratio_smoothed, 0.0, 1.0)
...
def fit_tau_proxy(v_series: np.ndarray,
                  ...
                  re_counts: np.ndarray | None = None) -> tuple[float, float, float, float, np.ndarray]:
    """fit phi and psi to the RE-based reply-pressure proxy while holding V(t) fixed"""
    v_arr = np.asarray(v_series, dtype=float)
    tau_target = np.asarray(tau_target, dtype=float)
    if re_counts is not None:
        weights = np.sqrt(np.asarray(re_counts, dtype=float) + 1.0)
    ...
    def loss(params: np.ndarray) -> float:
        ...
        weighted_loss, _r2 = tau_fit_metrics(tau_target, tau_model, weights)
    ...
    weighted_loss, r2 = tau_fit_metrics(tau_target, tau_fit, weights)
