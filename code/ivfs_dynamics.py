...
def build_tau_configurations(beta0: float,
    ...
    if reply_proxy is None:
        return {}

    tau_target = np.asarray(reply_proxy, dtype=float)
    ...
    phi_fit, psi_fit, fit_loss, fit_r2, tau_fit = fit_tau_proxy(fitted_v, tau_target, re_window)
    t_scenario_fit, scenario_results_fit = run_scenarios(beta0, gamma0, phi=phi_fit, psi=psi_fit)
    tau_configs['higgs_fit'] = {
        'label': 'Higgs RE proxy fit',
        'phi': phi_fit,
        'psi': psi_fit,
        'color': '#C62828',
        'tau_curve': tau_fit,
