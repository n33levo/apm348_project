...
SIR-style models and rumor models \cite{daleyKendall1964} give threshold and stability tools for diffusion, and recent work applies these ideas to social-media spread \cite{liShao2024}. For example, SVFR-type models focus on users' decisions and local network behavior. Toxicity-focused compartment models also exist \cite{maleki2022,addai2025}. The main gap is that most models compartmentalize \emph{users}; our model compartmentalizes \emph{content lifecycle states} and couples them to toxicity and user attrition, working at a more aggregate level.
...
\]
Here $\rho$ is content injection per user, $\mu_c$ natural content decay, and $\delta$ moderation/suppression. The point is a content-centric model that links amplification, toxicity, and user-base dynamics in one system.
...
\item \textbf{Benchmarking:} compare the reduced IVF helper model against a classical SIR fit on the same normalized Higgs window. In the current run, the IVF helper model gives the stronger spike-region fit ($R^2\approx 0.889$ versus $0.690$ for SIR), supporting the added lifecycle structure.
...
Network-aware and stochastic extensions are obvious follow-ups.
