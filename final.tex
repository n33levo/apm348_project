...
\item \textbf{Parameter fitting:} fit baseline IVF model to a 100-hour normalized retweet window (scale-invariant) using Nelder--Mead. Only $\beta_0,\gamma_0,\lambda_{\mathrm{bg}},V_0$ are data-fitted; remaining parameters are scenario assumptions. The toxicity coupling $\phi$ is calibrated from the Jigsaw toxicity dataset: $\phi = \hat{\tau}_{\text{empirical}} \cdot \psi / V^*(\alpha\!=\!0.5) \approx 0.056$.
...
\item \textbf{Uncertainty quantification:} parametric Poisson bootstrap ($B=500$) gives 95\% CIs for $\beta_0\in[1.463,1.487]$, $\gamma_0\in[0.152,0.155]$, and $\alpha(\mathcal{R}_0\!=\!1)\in[0.111,0.114]$. Profile likelihood surfaces confirm that both $\beta_0$ and $\gamma_0$ have well-defined minima (no flat ridges), supporting practical identifiability.
\end{itemize}

\section*{5. Expected Results and Conclusion Plan}
We expect higher amplification to improve short-run virality but worsen long-run toxicity and user retention. The conclusion will summarize equilibrium/stability findings, compare policy scenarios, discuss implications for platform moderation and ranking policy, and state limitations.

\paragraph{Limitations.}
Homogeneous mixing, deterministic dynamics, and single-index toxicity are the main structural simplifications.
The fitted $\beta_0$ and initial viral seed $V_0$ are jointly estimated from a single cascade; however, parametric bootstrap CIs are narrow (width $<2\%$ of the point estimate) and profile likelihood surfaces show well-defined minima for both $\beta_0$ and $\gamma_0$, lending practical identifiability support.
