...
\caption{Latent-pressure proxy comparison. In the current run the black reference curve is the selected RE proxy, and the same spread fit is combined with a baseline pressure block, an unconstrained same-dataset RE-based fit, and an external reference scale.}
...
This $\phi$ sweep is a \emph{local} robustness check around the baseline scenario block with $\psi$ held fixed at $0.1$, so it is designed to compare the current baseline $\phi=0.056$ against the external-reference scale $\phi\approx 0.1459$. It is not intended to reproduce the separate unconstrained RE-based fit, because that fit changes both $\phi$ and $\psi$ simultaneously and lands at the edge of the allowed parameter box.

For the same-dataset proxy family, the present implementation selects the RE proxy as the most interpretable and best-supported same-dataset target. The displayed same-dataset fit is the best unconstrained proxy fit, while the decay-based estimate is kept as a separate identifiability diagnostic. The current displayed RE-based fit gives approximately
\[
\phi \approx 0.9596, \qquad \psi \approx 1.0000,
\]
while the decay anchor gives approximately $\psi_{\mathrm{decay}} \approx 0.0409$. The fact that the best unconstrained fit pushes toward the parameter bounds is itself evidence that the pressure block is only partially identified. That is why the report treats the RE-based fit as a useful same-dataset proxy experiment rather than as a fully resolved toxicity calibration.
