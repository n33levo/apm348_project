...
\caption{Latent-pressure proxy comparison. In the current run the black reference curve is the selected RE proxy. Panel~(a) compares the proxy against the baseline pressure block, two same-dataset Higgs fits (decay-anchored and unconstrained), an external reference scale, and the reference-constrained configuration. The external-scale and reference-constrained curves share the same $\tau^*$ by construction (same $\phi/\psi$) and differ only in transient speed, as annotated in panel~(b). The Health-First case is subcritical in all setups, so its long-run pressure remains essentially zero.}
