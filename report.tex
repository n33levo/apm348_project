...
The spread benchmark against SIR also favors the current IVF helper model on the Higgs window. The repository additionally contains parametric bootstrap intervals and local profile-likelihood curves, both of which support practical identifiability of the spread parameters. To check whether the burst-decay shape is specific to the main spike only, the repository now also refits the reduced IVF helper model on three additional active 48-hour Higgs windows. Those appendix fits achieve $R^2$ values between roughly $0.69$ and $0.95$, which supports local robustness of the helper-model shape even though the shorter-window parameter estimates shift substantially across phases of the cascade.
...
\caption{Latent-pressure proxy comparison. In the current run the black reference curve is the selected RE proxy. Panel~(a) compares the proxy against the baseline pressure block, two same-dataset Higgs fits (decay-anchored and unconstrained), an external reference scale, and the reference-constrained configuration. In panel~(b), the reference-constrained setup is omitted because it shares the same long-run $\tau^*$ as the external-scale setup (same $\phi/\psi$) and differs only in transient speed. The Health-First case is subcritical in all setups, so its long-run pressure remains essentially zero.}
...
\textbf{Reference-constrained resolution.}
The identification problem has a principled partial resolution when an external toxicity
...
\]
which constrains the pair $(\phi,\psi)$ to the line $\phi = r\psi$ in parameter space.
Any point on that line gives the same long-run $\tau^* = r \cdot V^*_{\mathrm{mod}} \approx \tau_{\mathrm{ref}}$,
so the scale constraint alone does not select a unique point.
The \emph{reference-constrained} configuration resolves the remaining degree of freedom by
treating $\psi$ as the single free parameter on the constraint line and choosing the $\psi$ that
minimises the weighted proxy-fit loss directly.
This 1-D optimisation---with $\phi = r\psi$ enforced at every step---finds the point on the
constraint line that best explains the same-dataset RE proxy dynamics.
In the current run this gives
\[
\phi_{\mathrm{rc}} \approx 0.417, \qquad \psi_{\mathrm{rc}} \approx 0.286,
\]
with proxy-fit $R^2 \approx 0.41$.
This is better than both the decay-anchored ($R^2 \approx 0.10$) and plain
external-scale ($R^2 \approx 0.31$) configurations, confirming that $\psi=0.286$ is a better-supported
relaxation timescale than the default $\psi=0.1$, while the external reference level is still maintained.

One important consequence: because the reference-constrained and external-scale configurations
share the same gain ratio $r = \phi/\psi$, they produce \emph{identical} long-run $\tau^*$
and therefore identical bars in panel~(b) of Figure~\ref{fig:taucompare}.
Their difference is visible only in panel~(a): the reference-constrained curve ($\psi \approx 0.286$)
decays faster after the spike than the external-scale curve ($\psi = 0.1$).

The remaining gap between the reference-constrained ($R^2 \approx 0.41$) and
unconstrained ($R^2 \approx 0.74$) fits \emph{is} the identification problem.
The unconstrained fit achieves better proxy shape agreement but uses a scale
($\tau^*_{\mathrm{mod}} \approx 0.067$) inconsistent with the external reference.
Closing this gap would require either a dataset with both spread data and directly-observed
toxicity labels, or a multi-timescale $\tau$ model.
For the present course report, the reference-constrained configuration is the most defensible
single-point answer: it satisfies the external reference and finds the best proxy dynamics
consistent with that constraint. Appendix~B summarizes this identification split explicitly: Higgs identifies the spread block strongly and the pressure shape only partially, while the external reference fixes the pressure scale ratio rather than a unique $(\phi,\psi)$ pair.
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item The latent-pressure parameters $\phi$ and $\psi$ are only partially identified from the same-dataset proxy alone. The reference-constrained configuration gives the best $(\phi,\psi)$ pair consistent with the external reference constraint, but a fully resolved calibration would require either direct text-toxicity labels or a multi-timescale $\tau$ model.
...
python code/higgs_window_validation.py
\end{verbatim}
...
The current empirical package identifies different pieces of the model with different strengths:
\begin{itemize}[leftmargin=1.5em]
    \item \textbf{Higgs retweet counts} strongly identify the spread-side helper parameters $(\beta_0,\gamma_0,\lambda_0,d_\lambda,V_0)$ on the main 100-hour window.
    \item \textbf{Higgs reply activity} partially identifies the pressure \emph{shape}: it distinguishes better and worse relaxation timescales $\psi$, but only through a normalized proxy and therefore without fixing an absolute pressure scale.
    \item \textbf{The external Jigsaw reference} identifies the steady-state gain ratio $r=\phi/\psi=\tau_{\mathrm{ref}}/V^*_{\mathrm{mod}}$, so it constrains $(\phi,\psi)$ to a line rather than to one point.
\end{itemize}
The reference-constrained fit combines the second and third items in the most disciplined way currently available: it restricts $(\phi,\psi)$ to the externally anchored line $\phi=r\psi$ and then chooses the $\psi$ on that line that best matches the same-dataset reply proxy. What remains unresolved is the gap between that constrained optimum ($R^2\approx 0.41$) and the unconstrained same-dataset optimum ($R^2\approx 0.74$). That gap is not a plotting issue and not an optimization bug; it is the substantive identification limit of the present one-timescale pressure block.

\section{Additional Higgs active-window validation}
Because the Higgs record spans only 168 hours, independent 100-hour windows are not available. Instead, the repository now validates the reduced IVF helper model on three additional active 48-hour windows selected from the same record. These windows overlap, so they are not independent event replications; rather, they test whether the burst-decay helper model continues to describe different active phases of the cascade beyond the main 100-hour calibration window.

\begin{table}[H]
\centering
\caption{Additional active-window Higgs validation for the reduced IVF helper model.}
\label{tab:windowval}
\begin{tabular}{cccc}
    oprule
...
These additional fits show that the reduced IVF helper model continues to capture the local burst-and-decay shape well on later active windows, even though the fitted parameters shift across windows. That is the right interpretation for the present project: the helper model is a good local phenomenological fit to active cascade phases, but the most defensible global parameter estimates remain those from the main 100-hour calibration window used for the policy analysis.

\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{assets/higgs_window_validation.png}
\caption{Additional 48-hour active-window validation for the reduced IVF helper model. The local $R^2$ values remain strong on later active windows, supporting the burst-decay shape beyond the main calibration window.}
\label{fig:windowval}
\end{figure}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item independent-event spread validation beyond the current 168-hour Higgs record.
\end{enumerate}
